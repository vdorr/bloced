...
class BlockModel(object) :
	...
	def edit(prop_name, f) :
		def decorated(*v, **w) :
			getter = v[0].__getattribute__("_BlockModel__get_"+prop_name)
			old_meta = { prop_name : getter() }
			y = f(*v, **w)
			new_meta = { prop_name : getter() }
			v[0].__raise_block_changed(self, {"p":prop_name}, old_meta=old_meta, new_meta=new_meta)
			return y
		return decorated
...
#		if (b0, t0) in self.__connections :
	...
	def __on_block_changed(self, block, old_meta, event=None) :
...
class BasicBlocksFactory(BlockFactory) :
	...
	def __init__(self) :
		...
		self._BlockFactory__blocks += [
			...
			SBP("di", "Process IO", [ In("nr", W, .5), Out("y", E, .5) ], exe_name="io.di"),
			SBP("do", "Process IO", [ In("nr", W, .33), In("x", W, .66) ], exe_name="io.do"),
		]
