...
from itertools import groupby, chain, product, count, imap, ifilter
...
#	return graph, dict(ifilter(lambda c: not isinstance(c[0][0].prototype, JointProto), conns.iteritems()))
	return graph, { k : v for k, v in conns.iteritems() if not isinstance(k[0].prototype, JointProto) }
...
#			ifilter(lambda i: item_filter(*i), d.iteritems())))
	return { k_map(*i): v_map(*i) for i in d.iteritems() if item_filter(*i) }
...
#	conns0 = dict(ifilter(lambda item: item[1], model.connections.iteritems()))
	conns0 = { k: v for k, v in model.connections.iteritems() if v }
