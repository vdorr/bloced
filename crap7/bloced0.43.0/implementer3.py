...
#	s = map(lambda itm: itm[0], filter(lambda i: __no_preds(i[1].p), g.iteritems()))
	s = [ k for k, v in g.items() if __no_preds(v.p) ]
...
#			*[ dst for src, dst in filter(lambda i: i[0][0] == j[0], conns.iteritems()) ])))
		j_succs = list(set(chain(
			*[ dst for src, dst in filter(lambda i: i[0][0] == j[0], conns.items()) ])))
...
#			eltj = ifilter(lambda term: term.direction == OUTPUT_TERM and (v, term) in conns, v.terms)
...
#	return graph, dict(ifilter(lambda c: not isinstance(c[0][0].prototype, JointProto), conns.iteritems()))
	return graph, { k : v for k, v in conns.items() if not isinstance(k[0].prototype, JointProto) }
...
#			for key, values in groupby(sorted(it), key_selector) ])
	return { key : value_grouper([ value_selector(val) for val in values ])
			for key, values in groupby(it, key_selector) }

# ------------------------------------------------------------------------------------------------------------

def reverse_dict_of_lists(d, key_grouper) :
	return groupby_to_dict(
		chain(*[ product(v, [k]) for k, v in d.items() ]),
...
#	sys.exit(0)
	...
	s_adjl = groupby_to_dict(conns.items(),
		...
		xxxxx)#lambda values: set(chain(*values)))	# pack lists of successor blocks into sets
...
#	sys.exit(0)
	...
	g = { k : adjs_t(preds[k] if k in preds else set(), v) for k, v in succs }
