...
def __expand_joints(g, connections) :
	...
	for j in joints : #TODO ifilter(lambda i: isinstance(i[0].prototype, JointProto), g.iteritems()) :
		...
		#joint_out_terms = filter(lambda t: t.direction == OUTPUT_TERM, j[0].get_terms())
		
		#TODO make it set again
		j_succs = list(set(chain(
			*[ dst for src, dst in filter(lambda i: i[0][0] == j[0], conns.iteritems()) ])))

		joint_in_terms = filter(lambda t: t.direction == INPUT_TERM, j[0].get_terms())
...
def __expand_joints2(g, connections) :
	graph = dict(g)
	conns = dict(connections)
	joints = filter(lambda i: isinstance(i[0].prototype, JointProto), g.iteritems())
	#print "__expand_joints-graph-pre"; pprint(graph)
...
#	def key_transform(k, v) :
#		return new_key
#	
#	def value_transform(k, v) :
#		return new_value

	joints = ifilter(lambda i: isinstance(i[0].prototype, JointProto), g.iteritems())
	...
	conns2 = dict_map(conns,
		lambda k,v: ,
		lambda k,v: ,
		item_filter=lambda k,v: not isinstance(k[0].prototype, JointProto))

	g2 = dict_map(g,
		lambda k,v: ,
		lambda k,v: ,
		item_filter=lambda k,v: not isinstance(k.prototype, JointProto))
	
#	g2 = [ (k, v) for k, v in g.iteritems() ]
...
def __expand_delays(blocks, conns) :
	...
	def __expddel(nr) :
		i = BlockModel(DelayInProto())
		o = BlockModel(DelayOutProto())
		i.nr = o.nr = nr
		#TODO return expddel_t((i, i.get_terms()[0]), (o, o.get_terms()[0]))
...
#	f1 = lambda k: (
#		
#		(expd[k[0]].o, expd[k[0]].o.get_terms()[0])#expd[k[0]].o
#		if isinstance(k[0].prototype, DelayProto)
#		else k
#	)

#	f2 = lambda values: [
#		(expd[b].i, expd[b].i.get_terms()[0])#expd[k[0]].i
#		if isinstance(b.prototype, DelayProto)
#		else (b, t)
#			for b, t in values
#	]

#	conns2 = dict([ (f1(k), f2(v)) for k, v in conns.iteritems() ])
	...
	return list((set(blocks)-delays).union(chain(*expd.itervalues()))), conns2
...
def __preproc(model, meta) :
	...
	conns = dict(ifilter(lambda item: item[1], model.get_connections().iteritems()))
	#print("conns"); pprint(conns)

#	xx, yy = __expand_delays(blocks, conns)
#	print("xx"); pprint(xx)
#	print("yy"); pprint(yy)
...
#	return __preproc2(blocks, conns)
	...
	return __preproc2(*__expand_delays(blocks, conns))
