...
from serializer import pickle_dfs_model, unpickle_dfs_model
...
#		#XXX
	...
	def onMouseDownW(self, e) :
		...
		print self.editor.measure_selection()
...
#			l, t, _, _ = self.editor.measure_selection()
			
#			self.editor.move_selection(e.x-self.move_start.x, e.y-self.move_start.y)
#			self.start = e
...
#				print "m.orientation", m.orientation
	...
	def serialize_selection(self) :
		if self.selection :
#			print "TODO serialize_selection"
			
			#TODO move-line-without-reroute first, probably
...
#		print tkFont.families()
		...
		self.__add_top_menu("&File", [
			...
			("S&ave As...", "Ctrl+Shift+S", self.save_file_as),
#			"-",
#			("Export...", "Ctrl+E", self.__mnu_file_export),
		...
		self.__add_top_menu("&Edit", [
			...
			("&Redo", "Ctrl+Shift+Z", self.mnu_edit_redo),
			"-",
			("Cu&t", "Ctrl+X", self.mnu_edit_cut),
			("&Copy", "Ctrl+C", self.mnu_edit_copy),
			("&Paste", "Ctrl+P", self.mnu_edit_paste),
			...
			("U&ncomment Selection", "Ctrl+Shift+M", self.mnu_edit_uncomment),
			"-",
			("Pr&eferences", None, self.mnu_edit_preferences) ])

		menu_blocks = self.__add_top_menu("&Insert", [
			("&Insert last", "Ctrl+Shift+I", self.mnu_blocks_insert_last),
			"-" ])
