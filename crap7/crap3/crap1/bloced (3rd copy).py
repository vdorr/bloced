...
class Block(Canvas) :
	...
	def onMouseDown(self, e) :
		...
		return self.editor.blckMouseDown(self, e)#XXX
	...
	def onMouseMove(self, e) :
		...
		return self.editor.blckMouseMove(self, e)#XXX
	...
	def onMouseUp(self, e) :
		...
		return self.editor.blckMouseUp(self, e)#XXX
...
class BlockEditor(Frame, GraphModelListener) :
	...
	def blckMouseMove(self, sender, e) :
		if not sender.movingObject :
			return None
		xo, yo = tuple(self.canv.coords(sender.window))
		self.canv.coords(self.line, self.offset[0], self.offset[1],
			e.x + xo, e.y + yo)
	...
	def blckMouseUp(self, sender, e) :
		...
		dstx = e.x + xo0
		dsty = e.y + yo0
		target = self.canv.find_overlapping(dstx - 1, dsty - 1, dstx + 1, dsty + 1)
		if not target :
			return None
		...
		if term and term[0] in blck.window2term :
			...
			if blck == sender and srcterm == dstterm :
				return None
			if sender.editor.model.can_connect(sender.model, srcterm, blck.model, dstterm) :
				sender.editor.model.add_connection(sender.model, srcterm, blck.model, dstterm)
