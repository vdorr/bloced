...
#		block.graph = self #XXX ?
	...
	def add_connection(self, b0, t0, b1, t1, meta={}, deserializing=False) :
		...
		for listener in self.__listeners :
			listener.connection_added(b0, t0, b1, t1, deserializing=deserializing)
...
#		if (b0, t0) in self.__connections :
		...
		if (b0, t0, b1, t1) in self.__connections_meta :
			self.__connections_meta.pop((b0, t0, b1, t1))
		for listener in self.__listeners :
			listener.connection_removed(b0, t0, b1, t1)
	...
	def enum(self, deserializing=False) :
		...
		for src, targets in self.__connections.items() :
			for dst in targets :
				for listener in self.__listeners :
					listener.connection_added(*(src + dst), deserializing=deserializing)
	...
	def begin_edit(self) :
		# TODO nesting, only last one end_edit creates actual history frame
		self.__editing = True

		self.__history_frame_depth = None
