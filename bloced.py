...
#		print name, text, pos, size
		im = Image.new('RGBA', size, (0, 0, 0, 0)) # Create a blank image with the given size
		draw = ImageDraw.Draw(im)

		flipv, fliph, rot = self.model.orientation
		from mathutils import rotate4, make_rect, bounding_rect

#		points = [ rotate4(self.model.width/2, self.model.height/2, x, y, rot) for x, y in
#			make_rect(0, 0, size[0], size[1]) ]
		cx = self.model.width/2
		cy = self.model.height/2
#		cx, cy = (cy, cx) if self.model.orientation[2] % 180 else (cx, cy)
		points = [ rotate4(cx, cy, x, y, rot) for x, y in
			make_rect(0, 0, size[0], size[1]) ]
		sx = self.model.height/2
		sy = self.model.width/2
		sx, sy = (sy, sx) if self.model.orientation[2] % 180 else (0, 0)
		points2 = [ (x-sx, y-sy) for x, y in points ]
		brct_x, brct_y, brct_w, brct_h = bounding_rect(points2)
		
#		print self.model.prototype.type_name, self.model.orientation, (brct_x, brct_y, brct_w, brct_h)

#		sd = [W, N, E, S][self.model.orientation[2]/90]
#		print self.model.prototype.type_name, self.model.get_term_and_lbl_pos_alt(
#			False, 0.0, sd, 1,
#			0, size[0], size[1], center=True)
		lbl_x, lbl_y = self.model.get_label_pos(*size)
		print self.model.prototype.type_name, (lbl_x, lbl_y)

#rotate4(self.model.width/2, self.model.height/2, 0, 0, rot)
