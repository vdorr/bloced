...
class Block(Canvas) :
	...
	def update_text(self) :
		if type(self.model.prototype) == core.ConstProto :
			newtxt = self.model.value
		elif type(self.model.prototype) == core.DelayProto :
			newtxt = "Delay (%s)" % self.model.value if self.model.value != None else "None"
		else :
...
#		self.bind("<Configure>", self.onConfigure)
	...
	def __terms(self) :
		for t in self.model.terms :
			if t.variadic :
				for nr in range(self.model.get_term_multiplicity(t)) :
					yield t, nr
			else :
				yield t, None
	...
	def reshape(self) :
		...
		for t, nr in self.__terms() :
			term_tag = t.name
			print "reshape: t, nr=", t, nr
			if self.model.prototype.__class__ == core.ConstProto :
			...
			self.delete(term_tag)#XXX

			(x, y), (txtx, txty) = self.model.get_term_and_lbl_pos(t, nr, fnt.measure(t.name), txt_height)

			poly = self.get_term_poly(x, y, term_size, t.get_side(self.model), t.direction)
...
#			print self.itemcget(txt, "font").__class__
...
#				term_label = ""#str(self.model.value)
#				if t.variadic :
##XXX					term_label += "(%i)" % term_nr
#					pass
#			else :
#				term_label = t.name
#			if t.variadic :
##				print "Block.__init__:", self.model, t, self.model.get_term_multiplicity(t)
#				term_tag += "#" + str(nr)
#				pass

#			self.delete(t.name)#XXX

#			o = self.create_polygon(0, 0, fill="white", outline="black", tags=t.name)
#			self.bind_as_term(o)
#			txt = self.create_text(0, 0, text=term_label, anchor=NW, fill="black", tags=t.name)
##			print self.itemcget(txt, "font").__class__
...
#		#TODO precompute
#		sides = {
#			N : lambda p, tw: (self.model.width*p-term_size/2, 0, 0, term_size),
#			S : lambda p, tw: (self.model.width*p-term_size/2, self.model.height-term_size, 0, -term_size),
#			W : lambda p, tw: (0, self.model.height*p-term_size/2, term_size, 0),
#			E : lambda p, tw: (self.model.width-term_size-1, self.model.height*p-term_size/2, -1-tw, 0),
#			C : lambda p, tw: (0.5*self.model.width, 0.5*self.model.height, 0, 0),
#		}
#		fnt = tkFont.nametofont("TkDefaultFont")
#		txt_height = fnt.metrics("linespace")
#		for w, t  in self.window2term.iteritems() :
#			x, y, txtx, txty = sides[t.get_side(self.model)](t.get_pos(self.model), fnt.measure(t.name))
#			self.coords(w, *self.get_term_poly(x, y, term_size, t.get_side(self.model), t.direction))
...
#				print "term connected", sb, st, tb, tt
	...
	def __get_obj_at(self, dstx, dsty) :
		hits = self.canv.find_overlapping(dstx-1, dsty-1, dstx+1, dsty+1)
...
#				return None
		...
		if dstterm == None or (blck == sender and srcterm == dstterm) :
			return None
...
#		print "update_connection: ", sb, t0, tb, t1, line, path

		st, sn = t0 if isinstance(t0, tuple) else (t0, 0)
		tt, tn = t1 if isinstance(t1, tuple) else (t1, 0)

		s0 = st.get_location_on_block(sb, sn)
		tA = tt.get_location_on_block(tb, tn)
