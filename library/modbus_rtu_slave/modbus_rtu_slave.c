...
static int16_t* regs = NULL;//TODO check if set before use
...
void Serial_begin(uint8_t channel, unsigned long baud)
{
	Serial.begin(baud);
}

int Serial_available(uint8_t channel)
{
	return Serial.available();
}

size_t Serial_write_char(uint8_t channel, uint8_t c)
{
	return Serial.write(c);
}

int Serial_read_char(uint8_t channel)
{
	return Serial.read();
}
...
int send_reply(uint8_t channel, unsigned char *query, unsigned char string_length) 
{
        unsigned char i;
        ...
        for (i = 0; i < string_length; i++) {
                Serial_write_char(channel, query[i]);
        }
...
int receive_request(uint8_t channel, unsigned char *received_string) 
{
        int bytes_received = 0;

        /* FIXME: does Serial.available wait 1.5T or 3.5T before exiting the loop? */
        while (Serial_available(channel)) {
                received_string[bytes_received] = Serial_read_char(channel);
                bytes_received++;
...
int modbus_request(uint8_t channel, unsigned char slave, unsigned char *data) 
{
        int response_length;
        ...
        response_length = receive_request(channel, data);
...
int preset_multiple_registers(uint8_t channel, unsigned char slave,
unsigned int start_addr,
unsigned char count, 
unsigned char *query,
int *regs) 
{
        ...
        if (write_regs(start_addr, query, regs)) {
                status = send_reply(channel, packet, RESPONSE_SIZE);
        }
...
int write_single_register(uint8_t channel, unsigned char slave,
...
int read_holding_registers(uint8_t channel, unsigned char slave, unsigned int start_addr,
	unsigned char reg_count, int *regs) 
{
        ...
        status = send_reply(channel, packet, packet_size);

        return (status);
}


void configure_mb_slave(uint8_t channel, long baud, char parity, char txenpin, int16_t* registers)
{

	regs = registers;

        Serial_begin(channel, baud);
...
unsigned int regs_size) 
{
        ...
        unsigned int start_addr;
        int exception;
        int length = Serial_available(channel);
        ...
        length = modbus_request(channel, slave, query);
        if (length < 1)
                return length;
        ...
        if (exception) {
                        ...
                        errpacket);
                        send_reply(channel, errpacket, EXCEPTION_SIZE);
                        return (exception);
        } 
        ...
        switch (query[FUNC]) {
                case FC_READ_REGS:
                        return read_holding_registers(channel, slave, 
                        start_addr,
                        query[REGS_L],
                        regs);
                break;
                case FC_WRITE_REGS:
                        return preset_multiple_registers(channel, slave,
                        start_addr,
                        query[REGS_L],
                        query,
                        regs);
                break;
                case FC_WRITE_REG:
                        write_single_register(channel, slave,
                        start_addr,
                        query,
                        regs);
                break;                                
